...
static int procmisc(const char *name, int rnum){
  ...
  my_my_my_iprintf("checking versatile functions:\n");
...
static int proccompare(const char *name, int tnum, int rnum){
  my_my_my_iprintf("<Comparison Test>\n  seed=%u  name=%s  tnum=%d  rnum=%d\n\n",
  ...
  for(int t = 1; !err && t <= tnum; t++){
    ...
    my_my_my_iprintf("transaction %d (%s):\n", t, commit ? "commit" : "abort");
  ...
  my_my_my_iprintf("checking consistency of range:\n");
  for(int i = 1; i <= rnum; i++){
    char kbuf[RECBUFSIZ];
    int ksiz = sprintf(kbuf, "%d", i);
    int vsiz;
  ...
  my_my_my_iprintf("checking consistency on memory:\n");
  ...
  if(rnum > 250) my_my_my_iprintf(" (%08d)\n", inum);
  my_my_my_iprintf("checking consistency of hash:\n");
  inum = 0;
  ...
  if(rnum > 250) my_my_my_iprintf(" (%08d)\n", inum);
  my_my_my_iprintf("checking consistency of tree:\n");
