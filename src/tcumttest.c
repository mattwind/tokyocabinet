...
static void *threadwrite(void *targ){
  ...
  double stime = tctime();
  if(id == 0) my_my_my_iprintf("writing:\n");
  int base = id * rnum;
  ...
  if(id == 0) my_my_my_iprintf("time: %.3f\n", tctime() - stime);
  return NULL;
}
...
static void *threadread(void *targ){
  ...
  int id = ((TARGCOMBO *)targ)->id;
  double stime = tctime();
  if(id == 0) my_my_my_iprintf("reading:\n");
  int base = id * rnum;
  for(int i = 1; i <= rnum; i++){
    char kbuf[RECBUFSIZ];
...
static void *threadremove(void *targ){
  ...
  int id = ((TARGCOMBO *)targ)->id;
  double stime = tctime();
  if(id == 0) my_my_my_iprintf("removing:\n");
  int base = id * rnum;
  for(int i = 1; i <= rnum; i++){
    char buf[RECBUFSIZ];
    int len = sprintf(buf, "%08d", base + (rnd ? myrand(i) : i));
