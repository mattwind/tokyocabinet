...
static int procxstr(int rnum){
  my_my_my_iprintf("<Extensible String Writing Test>\n  seed=%u  rnum=%d\n\n", g_randseed, rnum);
  double stime = tctime();
...
static int proclist(int rnum, int anum, bool rd){
  my_my_my_iprintf("<List Writing Test>\n  seed=%u  rnum=%d  anum=%d  rd=%d\n\n",
  ...
  my_my_my_iprintf("record number: %u\n", tclistnum(list));
  sysprint();
  tclistdel(list);
  my_my_my_iprintf("time: %.3f\n", tctime() - stime);
  my_my_my_iprintf("ok\n\n");
  ...
  if(tr){
    ...
    while((kbuf = tcmapiternext(map, &ksiz)) != NULL){
      ...
      if(rnum > 250 && inum % (rnum / 250) == 0){
        iputchar('.');
        if(inum == rnum || inum % (rnum / 10) == 0) my_my_my_iprintf(" (%08d)\n", inum);
      }
      inum++;
    }
    if(rnd && rnum > 250) my_my_my_iprintf(" (%08d)\n", inum);
  }
...
/* perform mdb command */
static int procmdb(int rnum, int bnum, bool rd, bool tr, bool rnd, int dmode){
  my_my_my_iprintf("<On-memory Hash Database Writing Test>\n  seed=%u  rnum=%d  bnum=%d  rd=%d  tr=%d"
          "  rnd=%d  dmode=%d\n\n", g_randseed, rnum, bnum, rd, tr, rnd, dmode);
  double stime = tctime();
...
/* perform ndb command */
static int procndb(int rnum, bool rd, bool tr, bool rnd, int dmode){
  my_my_my_iprintf("<On-memory Tree Database Writing Test>\n  seed=%u  rnum=%d  rd=%d  tr=%d"
          "  rnd=%d  dmode=%d\n\n", g_randseed, rnum, rd, tr, rnd, dmode);
  double stime = tctime();
  TCNDB *ndb = tcndbnew();
  for(int i = 1; i <= rnum; i++){
